...
**append-only source of truth**; `wiki.md` is a **rebuildable cache**; the root
`log.md` is a **global timeline** across all topics; `MEMORY_INDEX.md` lists every topic.

Run all commands from the repo root:

    python3 theme_memory/scripts/cli.py <command> [args]

This skill only **adds** context (recall); it never deletes or hides anything, so
...
- **Refresh wiki** — after appending several entries to a topic → `summarize`.
...
Write/refresh a topic wiki. You generate the wiki text; omit `--content` for a
